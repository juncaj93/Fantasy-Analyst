...
import { SourceStateRepo, type SourceState } from './sourceState.ts';
 ...
 *
 * The shape is shared with every other published file this app reads — see
 * `repos/sourceState.ts`, which owns it — and re-exported under the injury name
 * because that is what the callers here have always called it.
 */
export type InjurySourceState = SourceState;
 ...
 * through every call.
 *
 * The fingerprint, the lease, the failure counter and the write ledger are
 * inherited rather than written here: the usage pipeline needs exactly the same
 * four mechanisms against a column-for-column identical table, and two copies
 * of a compare-and-swap lease is two things to get subtly different. What stays
 * below is what is genuinely about injuries — the events and the diff read.
 */
export class InjurySourceRepo extends SourceStateRepo {
  constructor(db: Database) {
    super(db, 'injury_source_state', 'injury_write_budget');
  }
