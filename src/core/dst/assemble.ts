/**
 * Everything the DST planner reads, gathered behind one interface.
 *
 * The same move `core/draft/boardBuilder.ts` and `core/matchup/build.ts` made
 * before it, and for the same reason: the *assembly* of a defence plan — which
 * teams to look up, which weeks to look them up for, how a stored line becomes
 * a week's anchor, what a roster spot costs — is not a fact about D1, and the
 * day it lives in a server service is the day Demo Mode has to grow a second
 * copy of it.
 *
 * So the three reads are an interface with three methods, `server/services/
 * dstPlanService.ts` satisfies it out of the schedule and betting repositories,
 * and a fixture satisfies it out of a static slate. Both then run this file,
 * which is the whole of the assembly, and `planDst` reaches every conclusion.
 *
 ...
 * nothing. See `outlook.ts`.
 */

import { evaluatePlayer, DEFENCE_POSITION, type StartSitInput } from '../startsit/engine.ts';
import { assessStreaming } from '../startsit/streaming.ts';
import { evaluateBench } from '../roster/bench.ts';
import { buildHeldPlayers } from '../roster/held.ts';
import { planDst, DST_PLAN, type DstOption, type DstPlan } from './planner.ts';
import { dstOutlook, forwardWeeks, type DstOutlook, type TeamForm } from './outlook.ts';
 ...
 */
export const EVENT_MATCH_HOURS = 36;

/**
 * The three reads, and nothing else.
 *
 * Every one of them is bounded by its arguments, and none of them is a fetch:
 * a live caller answers from rows some earlier refresh already paid for, and a
 * demo answers from a fixture.
 */
export interface DstPlanSources {
  /** The coming week's fixtures, for the deadline the advice expires at. */
  fixturesForWeek(season: string, week: number): Promise<ScheduleTeamWeek[]>;
  /** The planner's teams over the planner's weeks. */
  scheduleForTeams(
    season: string,
    teams: string[],
    range: { from: number; to: number },
  ): Promise<ScheduleTeamWeek[]>;
  /** What each offence has been priced at this season, for the fallback anchor. */
  impliedTotals(season: string, now: Date): Promise<Map<string, TeamForm>>;
}
 ...
 */
export async function assembleDstPlan(
  sources: DstPlanSources,
  request: DstPlanRequest,
): Promise<DstPlan | null> {
  ...
  const [thisWeekFixtures, teamSchedule, form] = await Promise.all([
    sources.fixturesForWeek(request.season, request.week).catch(() => [] as ScheduleTeamWeek[]),
    sources
      .scheduleForTeams(request.season, teams, {
        from: wanted[0] ?? request.week,
        to: wanted.at(-1) ?? request.week,
      })
      .catch(() => [] as ScheduleTeamWeek[]),
    sources.impliedTotals(request.season, now).catch(() => new Map<string, TeamForm>()),
  ]);
 ...
 */
function rosterCost(request: DstPlanRequest): {
  openSpots: number;
  dropCandidate: null | { playerId: string; name: string; position: string; surplus: number | null };
} {
