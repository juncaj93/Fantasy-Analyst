 ...
 *
 * Those are the two sides, and the balance between them was fixed. This makes
 * it a preference, in five positions, without making it a mystery: the market
 * side never moves, and the owner's side is scaled by one multiplier that the
 * board discloses in the same `score × weight = contribution` line every
 * component already prints.
 *
 ...
 *
 * The Settings screen needs the five positions at runtime, and a value import
 * of `engine.ts` here would put the whole ranking engine — tiers, survival,
 * concentration, opportunity — into the browser bundle to render one control
 * over a board the server builds. So this module holds no runtime reference to
 * the engine at all, and the caller supplies the weight table it is scaling.
 */
import type { DraftComponentWeights } from './engine.ts';

/**
 * The five positions, from "the market decides" to "my own reading decides".
 *
 ...
 * owner chose. The numbers live in one table below and nowhere else.
 */
