 ...
 * one ranking, one set of warnings, and the only difference between them is
 * where the rows came from. A second implementation for the demo would be a
...
export interface DraftBoardSources {
  ...
  adp: {
    ...
    latest(): Promise<BoardAdpSnapshot | null>;
...
/** The snapshot metadata a board prints. A superset is accepted. */
