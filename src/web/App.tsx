 ...
 */
const DraftScreen = lazy(() => import('./screens/DraftScreen.tsx').then((m) => ({ default: m.DraftScreen })));
...
export function App() {
  ...
  /*
   * Which decision the reader is looking at, remembered for the support button.
   *
   * Recorded here rather than inside each screen because the tab *is* the
   * decision for five of the six, and a `useEffect` in five components would be
   * five places to forget. The sixth — the defence — is a line rather than a
   * screen and records itself when it is opened; see `components/dst.tsx`.
   *
