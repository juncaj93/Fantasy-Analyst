 ...
 *     dropped on exit.
 *
 * What a scenario *is* now lives in `core/demo/placeholder/`: one in-season
 * Sunday served from captured responses. See that module for why.
 */

import type { DemoShowcase } from '../../core/demo/placeholder/index.ts';
/*
 * Type-only, so the demo's captured responses stay out of the initial bundle.
 *
 * `api.ts` imports this module on every page load — it has to, because it asks
 * whether a demo is running before every request. The type import is erased at
 * build time and the value arrives through the dynamic import in `enterDemo`.
 */
import type { DemoRuntime } from '../../core/demo/placeholder/runtime.ts';
...
export async function enterDemo(scenarioId: string): Promise<DemoSession> {
   ...
   *
   * Nobody who never opens Demo Mode should download even its names, and this module
   ...
   * `?demo=1` means.
   */
  const { DEFAULT_SHOWCASE_ID, findShowcase } = await import('../../core/demo/placeholder/index.ts');
  const wanted = scenarioId === '1' || scenarioId === 'on' ? DEFAULT_SHOWCASE_ID : scenarioId;
  const scenario = findShowcase(wanted);
  if (!scenario) throw new Error(`There is no demo scenario called "${wanted}".`);

  const { DemoRuntime } = await import('../../core/demo/placeholder/runtime.ts');
 ...
 *
 * Everything the demo touched is undone here: the stored selection and the
 * server's marker. What is
...
export async function exitDemo(): Promise<void> {
  ...
  setSession(null);
  writeStored(null);
  if (leaving) await markServer(false);
  announce();
}
