...
export function SmartTradeRow({ offer, onOpen }: { offer: OfferEvaluation; onOpen: () => void }) {
  ...
  return (
    <button type="button" className="smart-trade" data-testid="smart-trade-row" onClick={onOpen}>
      {/*
        Which reasoning produced this idea, when it was not the ordinary one.

        Absent on an upgrade, which is the overwhelming majority of the board
        and the shape a reader already has in their head — a tag on every row
        would be a tag that says nothing. It appears only for buy-low and
        sell-high, where the whole point is that the idea was *not* generated by
  ...
  return (
    ...
    >
      <div className="weekly" data-testid="smart-trade-detail-body" data-offer-id={offer.id}>
        ...
        */}
        {/*
          The arbitrage case, above the roster case, because it is the case.

          An upgrade's argument is the lineup points two lines up; a buy-low's
          argument is that the price and the player have come apart, and the
          lineup points are close to zero *by construction* — that is the gate
