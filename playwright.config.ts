 ...
 * `mobile-chromium-*` projects exist so the same specs can run in environments
 * where the WebKit build cannot be downloaded; they are a fallback, not a
 * replacement — CI runs the WebKit projects.
 *
 *   npm run e2e            # WebKit, all three widths
 *   npm run e2e:chromium   # fallback engine, all three widths
 */
...
/** The three portrait widths called out in docs/06_UI_AND_QA.md. */
const VIEWPORTS = [
